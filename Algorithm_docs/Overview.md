# Overview：module_pw 模块的旧算法与实现方式详解
...
- XY:
    我的感受是 GPT-5.5(xhigh) 非常强而且非常快，给出明确指令后，它不到2分钟就完成了对应模块的阅读，并找出了可改进的方向。我可以通过它写的文档了解每个函数的作用，并且初步判断它给出的算法改进方向的合理性。不过它的语言冷峻到不像正常人，倾向于用“不是...而是...”的句式也可能是 RLHF 导致的。
- XXT：
    我主要使用的是Copilot学生认证提供的GPT-5.2-Codex以及Gemini 3.1 Pro。目前看来，GPT模型生成的内容简明概要，能很好帮我理解算法的具体实现细节以及可能的改进方向；美中不足的是它的语言格式有时过于琐碎，导致阅读起来不太通畅；Gemini模型生成的内容则较为丰富详细，似乎在循循善诱地教你，缺点则在于对算法的理解和解释不够清晰，经常绕圈子。不过总体而言，AI工具确实极大提升了我们阅读、理解和改进代码的效率。
- YY:
    本次作业通过撰写 skill 让 AI(Deepseek-v4-pro) 阅读代码并生成文档，后续人工进行补充和修改完成。AI 生成的文档差强人意，不过有个神奇的问题是 latex 公式中的 `\t` （如 `\text`，`\times`）和 `\r` （如 `\rfloor`）在输出中消失了，推测是对转义字符的处理问题。
