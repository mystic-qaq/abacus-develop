...
usage() {
    cat <<'EOF'
Usage:
  run_task5_simd_benchmark.sh --case-dir DIR --label LABEL --out-dir DIR [options]

Required:
  --case-dir DIR        ABACUS test case directory.
  --label LABEL         Result label, e.g. baseline or simd.
...
Examples:
  ./run_task5_simd_benchmark.sh \
      --repo-dir . \
      --build-dir build \
      --case-dir homework_docs/test_cases/gaas_medium \
      --label baseline \
      --nproc 1 \
      --threads 8 \
      --repeat 5 \
      --out-dir homework_docs/test_cases/task5_runs/baseline_gaas_medium_np1_omp8
EOF
}
...
compiler_c=$(grep -E '^CMAKE_C_COMPILER:FILEPATH=' "$build_dir/CMakeCache.txt" 2>/dev/null | head -n1 | cut -d= -f2- || true)
compiler_cxx=$(grep -E '^CMAKE_CXX_COMPILER:FILEPATH=' "$build_dir/CMakeCache.txt" 2>/dev/null | head -n1 | cut -d= -f2- || true)
compiler_c_id=$(grep -E '^CMAKE_C_COMPILER_ID:STRING=' "$build_dir/CMakeCache.txt" 2>/dev/null | head -n1 | cut -d= -f2- || true)
compiler_cxx_id=$(grep -E '^CMAKE_CXX_COMPILER_ID:STRING=' "$build_dir/CMakeCache.txt" 2>/dev/null | head -n1 | cut -d= -f2- || true)
compiler_cxx_ver=$(grep -E '^CMAKE_CXX_COMPILER_VERSION:STRING=' "$build_dir/CMakeCache.txt" 2>/dev/null | head -n1 | cut -d= -f2- || true)
...
printf 'label,case_name,git_branch,git_commit,nproc,threads,run_kind,run_index,stdout_path,stderr_path,exit_code,start_time,end_time\n' > "$run_csv"

{
    echo "Task5 benchmark start: $timestamp"
...
run_one() {
    ...
    printf '%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s\n' \
        ...
        "$run_kind" "$run_index" "$stdout_file" "$stderr_file" "$exit_code" \
...
python3 "$script_dir/collect_task5_simd_results.py" \
    --input-dir "$out_dir" \
    --out-dir "$out_dir"
