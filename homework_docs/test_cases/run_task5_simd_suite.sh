...
# Fixed Task 5 suite configuration.
...
usage() {
    cat <<'EOF'
Usage:
  run_task5_simd_suite.sh --label LABEL [--build-dir DIR | --abacus-bin PATH] [options]

Required:
  --label LABEL         Result label, usually baseline or simd.

Executable selection:
  --build-dir DIR       Build directory passed through to run_task5_simd_benchmark.sh.
  --abacus-bin PATH     ABACUS executable passed through to run_task5_simd_benchmark.sh.

Optional:
  --repo-dir DIR        Repository root. Default: auto-detect from script location.
  --out-dir DIR         Suite output directory.
                        Default: homework_docs/test_cases/task5_suite_runs/$label_$timestamp
...
if [[ -z "$OUT_DIR" ]]; then
    OUT_DIR="$REPO_DIR/homework_docs/test_cases/task5_suite_runs/${LABEL}_${timestamp}"
fi
mkdir -p "$OUT_DIR"
...
{
    echo "Task5 suite start: $(date '+%Y-%m-%d %H:%M:%S %z')"
...
for case_rel in "${CASES[@]}"; do
    ...
    for nproc in "${NPROCS[@]}"; do
        for thread in "${THREADS[@]}"; do
            ...
            cmd=(
                bash "$SCRIPT_DIR/run_task5_simd_benchmark.sh"
                --repo-dir "$REPO_DIR"
                --case-dir "$case_dir"
                --label "$LABEL"
                --nproc "$nproc"
...
python3 "$SCRIPT_DIR/collect_task5_simd_suite_results.py" \
    --suite-dir "$OUT_DIR" \
    --out-dir "$OUT_DIR"

echo "Task5 suite finished. Results are under: $OUT_DIR"
