...
- 在WorkflowB中使用GPT的路径比较陡峭，开始它的实验设计只用了NaCl 1个原子体系且设置了迭代步数为3，我认为步数过少时测试结果可能误差较高，NaCl 1个体系也未必有很强的代表性，因此建议它增加步数到10以及多选几种合适的原子体系。但是不知道是偶然降智还是上下文过长，它没有按预想地执行任务，结果很差。不过好在在几轮对话修正之后最终还是完成了任务。
