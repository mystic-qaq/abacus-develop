...
由于还未实现 GammaOnly，下面的指标用于后续对照和 sanity check，不作为优化收益结论。当前 performance baseline 如下：
...
- 本次实测是单节点小用例，目标只是得到 correctness smoke 以及 baseline，不用于评估性能收益。
...
- 关于GammaOnly这条分支的测试，起初GPT仅测试了代码的正确性，之后在人为提醒下又测试了一些已经可测的性能baseline。同时它也反过来提醒我，目前能做的仅是full-complex静态规模baseline，在compact未实现前大部分指标仍处于无法测试的状态。