...
本次在 `GammaOnly` 分支完成了工作流 D 第 12 周阶段目标：规划全 Gamma 与混合 k 点正确性验证，运行当前分支的现状基线，并明确 GammaOnly 紧凑存储前后的对比指标。

当前分支的核心状态是：`module_pw` 内部已有 `PW_Basis` / `PW_Basis_K` 的半谱 FFT 基础，但真实 PW 生产路径尚未启用 GammaOnly。输入层在 `basis_type=pw` 且 `gamma_only=1` 时会把参数重置为 `0`，`setup_pwrho` 与 `setup_pwwfc` 仍硬编码以 `false` 初始化 PW 基组。因此，本报告中的实测数据是当前 full-complex 路径的 correctness baseline，不是 GammaOnly 性能收益数据。
...
| 汇总表 | `tables/correctness_summary.csv`、`tables/parallel_comparison.csv` |
...
每个 case 跑 `np1_omp1` 与 `np4_omp1`。脚本会把 `pseudo_dir` 改成绝对路径，避免 benchmark 目录层级改变导致找不到赝势。
...
第一阶段是安全 fallback：当 k 点集合含 Gamma 与非 Gamma 点时，程序保持 full-complex 路径，保留用户 KPT，并输出降级原因。当前 `PW_Basis_K` 层因为 `kmaxmod > 0` 会关闭全局 `gamma_only`，数值上安全；但输入层对 PW 的 `gamma_only=1` 会覆盖 KPT，后续需要改成“保留 KPT 并降级”。
...
`peak_ratio` 尤其重要。如果为了兼容旧接口长期保留 expanded temporary，峰值内存可能明显低于理论收益，需要如实写入报告。
...
- 本次实测是单节点小用例，目标是 correctness smoke，不用于评估性能收益。
- 当前生产路径未启用 PW GammaOnly，因此没有 compact memory saving 实测数字。
